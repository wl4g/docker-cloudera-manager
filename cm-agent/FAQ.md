...
## 1. Error when starting `cm-agent` container: `Failed! trying again in 5 second(s): [Errno 111] Connection refused`
...
- ***Solve:*** This is usually caused by a failure to start the supervisored service, then the cause of its failure may be the configuration file in container: `/etc/supervisord.conf` and `/run/cloudera-scm-agent/supervisor/supervisord.conf` are incorrect, etc. (The path mapped to the host is: `/etc/cloudera-scm-agent/supervisord.conf`)

- If **cm-agent** starts normally, the process tree should basically look like this:
